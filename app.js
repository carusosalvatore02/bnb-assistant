// app.js - B&B Assistant Web App
// Usa fetch con credentials:'include' per leggere i cookie del gestionale

const GESTIONALE = 'https://www.bed-and-breakfast.it';
const EXPORT_URL = GESTIONALE + '/ar/prenotazione_azioni.cfm?azione=ExportExcel&anno=2026';
...
function bindEvents() {
  ...
  // Nasconde input-row quando non è sulla tab chat
...
function loadFromStorage() {
  try {
    const raw = localStorage.getItem('bnb_bookings');
    const ls = localStorage.getItem('bnb_lastSync');
    if (raw) {
...
function doSync() {
  ...
  setStatus('loading', 'Download prenotazioni dal gestionale…');

  // Trova l'URL di esportazione dinamicamente oppure usa quello di default
  findExportUrl(function(url) {
    fetch(url, { credentials: 'include', mode: 'cors' })
      .then(function(resp) {
        if (resp.status === 403 || resp.status === 401 || resp.redirected) {
          throw new Error('non_auth');
        }
      ...
      .catch(function(err) {
        btn.classList.remove('loading');
        if (err.message === 'non_auth' || err.message.includes('Failed to fetch') || err.message.includes('NetworkError') || err.message.includes('CORS')) {
          showOnboarding();
          setStatus('err', 'Accedi prima al gestionale, poi riprova');
        } else {
          setStatus('err', 'Errore: ' + err.message);
        }
      });
  });
}

function findExportUrl(callback) {
  // Prova a trovare l'URL corretto facendo una richiesta alla pagina prenotazioni
  // e cercando il link esporta, oppure usa l'URL di default
  callback(EXPORT_URL);
}
...
async function parseXlsx(u8) {
  ...
  if (sheetKey) sheetRaw = files[sheetKey];
...
async function readZip(u8) {
  ...
  for (var e=0;e<cdCount;e++){
    ...
    entries.push({fname,method,compSize,uncompSize,lhOffset});
...
function processData(rows) {
  bookings = rows.map(function(r) {
    return {
      ...
      canale: r['Canale']||'',
      checkin: excelDate(r['Check-in']),
      checkout: excelDate(r['Check-out']),
      nome: r['Nome']||'',
      cognome: r['Cognome']||'',
      paese: r['Paese']||'',
      importo: parseFloat(r['Importo'])||0,
      commissioni: parseFloat(r['Commissioni'])||0,
      tassa: parseFloat(r['Tassa soggiorno'])||0,
      camera: r['Camera']||'',
      adulti: parseInt(r['Numero adulti'])||0,
      bambini: parseInt(r['Numero bambini'])||0,
      stato: r['Stato']||''
    };
  });
...
function showTab(name) {
  ...
  // Mostra input-row solo nella tab chat
  document.getElementById('input-row').style.display = name==='chat' ? 'flex' : 'none';
  // Allinea chat-wrap all'altezza disponibile
...
function showTyping(){
  var c=document.getElementById('messages');
  var d=document.createElement('div');d.id='tdot';d.className='msg ai';
...
function openReport(type){
  ...
  var title=document.getElementById('rmodal-title');
...
function printReport(){
  var b=document.getElementById('rmodal-body').innerHTML;
  var t=document.getElementById('rmodal-title').textContent;
  var w=window.open('','_blank');
  w.document.write('<!DOCTYPE html><html><head><title>'+t+'</title><style>body{font-family:sans-serif;padding:20px;font-size:13px}h2{margin-bottom:8px}.rstitle{font-weight:600;font-size:11px;text-transform:uppercase;color:#666;margin:14px 0 6px;border-bottom:1px solid #ddd;padding-bottom:4px}.rsrow{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee}.rsbadge{padding:2px 8px;border-radius:10px;font-size:11px}.b-bk{background:#E6F1FB;color:#185FA5}.b-ok{background:#E1F5EE;color:#0F6E56}.b-ko{background:#FAECE7;color:#993C1D}.totbox{background:#f5f5f2;padding:10px;margin-top:8px}.totrow{display:flex;justify-content:space-between;padding:3px 0}.totrow.main{font-weight:700;border-top:1px solid #ddd;margin-top:6px;padding-top:6px}</style></head><body><h2>'+t+'</h2>'+b+'</body></html>');
...
function reportSettimanale(today,todayStr,label){
  ...
  var active=bookings.filter(function(b){return b.stato==='Attiva'||b.stato==='Modificata';});
  var rev=active.reduce(function(s,b){return s+b.importo;},0);
  var comm=active.reduce(function(s,b){return s+b.commissioni;},0);
  var tasse=active.reduce(function(s,b){return s+b.tassa;},0);
